...
curl -LO https://github.com/kubernetes/kops/releases/download/v1.17.0/kops-linux-amd64
chmod +x kops-linux-amd64

sudo mv kops-linux-amd64 /usr/local/bin/kops
sudo apt-get install nano curl python3-pip -y
...
kops create cluster --node-count=2 --master-size="t3.medium" --node-size="t3.medium" --master-volume-size=30 --node-volume-size=30 --zones $az $clname
